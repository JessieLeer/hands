<template>
  <van-row class='main'>
	  <van-col :span='24'>
		  <cheader backcolor='#1AC69C' title='' v-bind:ckeyword='order.keyword' v-bind:hasSearch='true' placeholder='搜索供应商名称或助记码' v-on:search='search' v-on:input='onInput'></cheader>
          ...
          <van-cell v-for='(item,index) in popup.data' v-bind:key='index' v-bind:title="item" v-on:click='search(item)'/>
			...
			<van-pull-refresh v-model="isLoading" @refresh="onRefresh" class='orders'>
				<van-list v-model="loading" v-bind:finished="finished" finished-text="没有更多了" @load="onLoad">
					<van-cell v-for='(item, index) in order.data' v-bind:key='index' v-bind:title="item.company" is-link class='order' v-bind:url='type == "nobone" ? `/#/picking/good/index/${item.id}` : `/#/picking/good/index1/${item.id}`'>
						<template slot="label">
